    function [maps,Wlarge, Err]=objan_campomedio_flo2(data,xdat,zdat,params,xmap,zmap);
%%%%function [maps,Wlarge,datamean]=objan_campomedio_flo(data,xdat,zdat,params,xmap,zmap);
...
% JC 05/2016
% Modificado para el estrecho de la Florida
% CdC JC 2/06/2016
  
%calcula parametros para transporte
%f1=50*60/27; centlat=23.9052; f2=cos(pi/180*centlat);
f1=50*60/27; % centlat=20.5; f2=cos(pi/180*centlat)*1000;
f2=1000; % para convertir km a m
% factor de escala por la orientacion de la seccion, por consistencia
orisec=13.7825; cosori=cosd(orisec); 
facesc = f1*f2/cosori;
...
expdl=exp(- (px1+pz1)); 
%%%%maps = expdl'*Wlarge + datamean; %size(expdl)
maps = expdl'*Wlarge;
